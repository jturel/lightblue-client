...
describe 'projections' do
  ...
  let(:field_projection) { Lightblue::Projection.new(:foo) }
  let(:match_query) { Expression.new.apply_ast(query_exp_node) }
  let(:match_projection) { Lightblue::Projection.new(:foo).match(match_query).include}
  ...
  describe 'projections' do
    describe 'field projections' do
      it 'to_ast' do
        ...
        assert_ast_equal expected, field_projection.to_ast
      end
    ...
    describe 'array match projections' do
      it 'to_ast' do
        ...
        assert_ast_equal expected, p.to_ast
      end
