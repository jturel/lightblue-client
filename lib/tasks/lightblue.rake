...
namespace :lightblue do
  ...
  task :init do
    raise 'Set LIGHTBLUE_HOST and LIGHTBLUE_METADATA_PATH env variables to run' if !ENV['LIGHTBLUE_HOST'] && !ENV['LIGHTBLUE_METADATA_PATH']
  ...
  namespace :entity do
    ...
    task :versions => options do
      entity = ENV['ENTITY']
      raise 'Must define env["ENTITY"]' if entity.nil?
